...
namespace Main {
  export class Lava extends Phaser.TileSprite {
    ...
    constructor(state: State, start: number, speed: number) {
      super(state.game, 0, state.world.height + 32, state.camera.width, state.camera.height, 'environment', 'lava');
      this.state = state;
      ...
      let fire = new Phaser.TileSprite(state.game, 0, -32, state.camera.width, 32, 'environment', 'flames/1');
